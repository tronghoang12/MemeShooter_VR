...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	struct v2f {
		...
		float3 nor_world : TEXCOORD2;
		#if SPECULAR && !NORMAL_MAP && !DIRLIGHTMAP_COMBINED && !FLAT
			...
			#endif
		#endif
		#if REFLECTION && !NORMAL_MAP && !FLAT
			float3 viewReflectDir : TEXCOORD6;
		#endif
		#if NORMAL_MAP && ((DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED) \
			|| SPECULAR_HAIR
			...
			#else
				float fogCoord : TEXCOORD13;
			#endif
		#endif
		#if DIFFUSE && !DIFFUSE_PER_PIXEL && !FLAT && !LIGHTMAP_ON && (VERTEXLIGHT_ON || _ADDITIONAL_LIGHTS)
			half3 col_diffuse_add : TEXCOORD14;
		#endif
	...
	struct v2f {
		float4 pos : SV_POSITION;
		float4 uv : TEXCOORD0;
		#if VERTEX_COLORS || DETAIL_VERTEX_COLORS
			half4 color : COLOR;
		#endif
		float3 pos_world : TEXCOORD1;
		float3 nor_world : TEXCOORD2;
		...
		#endif
		#if MATCAP
			#if !(NORMAL_MAP || NORMAL_MAP_TOP) && !FLAT
				float3 nor_view : TEXCOORD5;
			#elif MATCAP_PERSPECTIVE
				float3 viewDir_view : TEXCOORD5;
			#endif
...
float3 PlantSway(appdata_full v) {
	float3 pos = v.vertex.xyz;
		...
		float3 worldPosCenter = mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz;
...
// LAYER FUNCTIONS ////////////////////////////////////////////////////////////////////////
#if LAYER1
	half4 Layer1(half4 col, float2 uv, half4 vertexColor) {
...
#if LAYER2
	half4 Layer2(half4 col, float2 uv, half4 vertexColor) {
...
#if LAYER3
	half4 Layer3(half4 col, float2 uv, half4 vertexColor) {
...
v2f vert (appdata_full v) {
	...
	#if (!TRIPLANAR && \
			NORMAL_MAP && ((DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED)) \
		|| SPECULAR_HAIR
			o.tan_world = float4(UnityObjectToWorldDir(v.tangent.xyz), v.tangent.w);
	#endif
	#if DIFFUSE && !DIFFUSE_PER_PIXEL && !FLAT && !LIGHTMAP_ON && (VERTEXLIGHT_ON || _ADDITIONAL_LIGHTS)
		o.col_diffuse_add = AdditionalLights(o.pos_world, o.nor_world, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast);
	#endif
	#if SPECULAR && !(NORMAL_MAP || NORMAL_MAP_TOP) && !DIRLIGHTMAP_COMBINED && !FLAT && !SPECULAR_HAIR
		o.specReflectDir = normalize(reflect(-_WorldSpaceLightPos0.rgb, o.nor_world));
	#endif
	#if RIM || SPECULAR || REFLECTION
		o.viewDir_world = normalize(WorldSpaceViewDir(v.vertex));
	#endif
	#if MATCAP
		#if !(NORMAL_MAP || NORMAL_MAP_TOP) && !FLAT
...
half4 frag (v2f i) : COLOR {
	...
	#endif
	#if MATCAP && MATCAP_PERSPECTIVE && ((NORMAL_MAP || NORMAL_MAP_TOP) || FLAT)
	...
	// Calculate intermediate vectors
			...
			#endif
			#if (DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP || DIRLIGHTMAP_COMBINED
				float3 bumpNor_world = normalize(norX.zyx * blend.x + norYtop.xzy * blend.y + norZ.xyz * blend.z + norYbottom * blend.w);
			#endif
		#endif
	#endif
	#if (DIFFUSE && !LIGHTMAP_ON) || RIM || SPECULAR || MATCAP
		#if (NORMAL_MAP || NORMAL_MAP_TOP)
	...
	#endif
	#if ((NORMAL_MAP || NORMAL_MAP_TOP) || SPECULAR) && DIRLIGHTMAP_COMBINED
	...
	// BASE
		...
		col_base.a = 1;  // Triplanar alpha has artifacts if blended, so clamp at 1
		col_base = ColorBrightness(col_base, _Brightness, _Color);
		// BASE TOP
	...
	#if !TRIPLANAR
		#if LAYER1
			col_base = Layer1(col_base, uv, i.color);
		#endif
		#if LAYER2
			col_base = Layer2(col_base, uv, i.color);
		#endif
		#if LAYER3
			col_base = Layer3(col_base, uv, i.color);
		#endif
	...
	// LIGHTING
		...
		half3 col = 1;
	#endif
	#if DIFFUSE && !LIGHTMAP_ON
		half ndotl = max(0, dot(_WorldSpaceLightPos0.xyz, adjNor_world));
		ndotl = max(0, lerp(ndotl, 1, _DiffuseWrap));
		ndotl = pow(ndotl, _DiffuseContrast) * _DiffuseBrightness;
		half3 col_diffuse = ndotl * _LightColor0.rgb;
		#if !DIFFUSE_PER_PIXEL && !FLAT && (VERTEXLIGHT_ON || _ADDITIONAL_LIGHTS)
			col_diffuse += i.col_diffuse_add;
		#else
			col_diffuse += AdditionalLights(i.pos_world, adjNor_world, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast);
		#endif
		...
		#if DIRLIGHTMAP_COMBINED && (NORMAL_MAP || NORMAL_MAP_TOP)
		...
		#endif
		#if !SPECULAR_HAIR
			#if (NORMAL_MAP || NORMAL_MAP_TOP) || DIRLIGHTMAP_COMBINED || FLAT
		...
		#endif
		if (dot(_SpecularColor.rgb, 1) != 3)  // Specular Color
	...
	// Shadows
		...
		#endif
		if (lum < thresLower1)
			animeColor = _AnimeColor1;
		#if ANIME_SOFT
			else if (lum < thresUpper1)
				animeColor = lerp(_AnimeColor1, _AnimeColor2, (lum - thresLower1) / (_AnimeSoftness * 2));
		#endif
		else if (lum < thresLower2)
			animeColor = _AnimeColor2;
		#if ANIME_SOFT
			else if (lum < thresUpper2)
				animeColor = lerp(_AnimeColor2, _AnimeColor3, (lum - thresLower2) / (_AnimeSoftness * 2));
		#endif
		else
			animeColor = _AnimeColor3;
		col = animeColor.rgb;
		if (dot(col, 1) > 0.00001) 
			col *= normalize(col);
		// Blend light to base
		col *= col_base.rgb;
	#endif
